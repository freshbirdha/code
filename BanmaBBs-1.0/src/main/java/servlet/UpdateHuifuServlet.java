...
public class UpdateHuifuServlet extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if(huifu.getUid() != loginUser.getUid()) {
			//给予错误提示
			request.setAttribute("message", "您没有修改权限 <a href=\"tiezi?tid="+huifu.getTid()+"\"> 》》》去帖子详情页</a>");
			request.getRequestDispatcher("/WEB-INF/view/error.jsp").forward(request, response);
			return;
		}
		...
		if(huifuDao.updateHuifu(huifu)) {
			response.sendRedirect("tiezi?tid="+huifu.getTid());
		}else {
			request.setAttribute("message", "修改失败，请重新尝试 <a href=\"tiezi?tid="+huifu.getTid()+"\"> 》》》去帖子详情页</a>");
			request.getRequestDispatcher("/WEB-INF/view/error.jsp").forward(request, response);
		}
